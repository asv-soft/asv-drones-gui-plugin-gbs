...
namespace Asv.Drones.Gui.Plugin.Gbs
{
    public abstract class GbsRttItem : ViewModelBase, IGbsRttItem
    {
        ...
        public GbsRttItem():base(WellKnownUri.UndefinedUri)
        {
        
        }
    
        protected GbsRttItem(IGbsClientDevice baseStation,string name) : base($"{WellKnownUri.ShellPageMapFlightWidget}.gbs.{name}")
        {
            IsVisible = true;
